...
//            return $request->input('lines');
            ...
            foreach ($request->input('lines') as $lineData) {
                ...
                AggregatedInvoiceLines::create([
                    ...
                    'description' => $lineData['description'] ?? ($lineData['label'] ?? $product->label),
                    'price' => $lineData['price'],
                    'type' => $lineData['type'],
                    'TVA' => $lineData['TVA'] ?? 0,
                    'reference' => $lineData['reference'] ?? ($product->ref ?? null),
...
//            DB::rollBack();
    ...
    public function edit(AggregatedInvoice $invoice)
    {
        abort_if(Gate::denies('access-dashboard'), Response::HTTP_FORBIDDEN, '403 Forbidden');

        return view('backOffice.aggregatedInvoices.edit', compact('invoice'));
    }
